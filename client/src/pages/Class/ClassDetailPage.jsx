...
import { IconCheck, IconChevronLeft, IconChevronRight, IconArrowLeft, IconBrandYoutube } from '@tabler/icons-react';
...
function getClassData(moduleId) {
  const STORAGE_KEY = 'finlitgo_classes';
  const saved = localStorage.getItem(STORAGE_KEY);
  const classes = saved ? JSON.parse(saved) : [];
  return classes.find(c => String(c.id) === String(moduleId));
}
...
export default function ClassDetailPage() {
  ...
  // Trigger streak when entering to study a module
  useEffect(() => {
    recordStudyActivity();
  }, []);

  // Try to load from localStorage (admin CRUD data)
  const classData = getClassData(moduleId);
  ...
  const quizQuestions = classData?.quizzes?.length > 0 
    ? classData.quizzes 
    : [
        { question: 'What percentage of your income should go to "Needs" in the 50/30/20 rule?', options: ['20%', '30%', '50%', '10%'], correctAnswer: 2 },
        { question: 'Which of the following is considered a "Want"?', options: ['Rent', 'Groceries', 'Dining Out', 'Electricity'], correctAnswer: 2 }
      ];

  const ytId = classData ? getYouTubeId(classData.youtubeUrl) : null;
  const classTitle = classData?.title || 'Money Management Basics';
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto min-h-[calc(100vh-200px)] py-8 px-6">
      {/* Sidebar Overview */}
      <div className="lg:w-80 bg-[#1A1A1A] border border-zinc-800 rounded-2xl p-6 hidden lg:flex flex-col h-fit sticky top-[100px]">
        ...
        <h3 className="font-bold text-lg text-white font-orbitron mb-4">{classTitle}</h3>
        
        {/* YouTube Embed Preview in Sidebar */}
        {ytId && (
          <div className="mb-4 rounded-xl overflow-hidden border border-zinc-700 aspect-video">
            <iframe 
          ...
          {[1,2,3,4].map((step, i) => (
            <div key={i} className="flex gap-4 items-start relative bg-zinc-900/50 p-3 rounded-xl border border-transparent hover:border-zinc-700 cursor-pointer group transition-colors">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 ${
                i === 2 ? 'bg-gradient-to-br from-violet-600 to-purple-400 text-white shadow-[0_0_10px_rgba(124,58,237,0.3)]' : 
                i < 2 ? 'bg-green-500/20 text-green-500 border border-green-500/30' : 
              ...
              <div className="flex-1 mt-1">
                ...
                {i === 2 && <span className="text-xs text-violet-400 font-mono mt-1 block">Reading...</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      >
        ...
        {ytId && (
          <div className="w-full aspect-video border-b border-zinc-800">
            <iframe 
              src={`https://www.youtube.com/embed/${ytId}`}
              className="w-full h-full"
        ...
        <div className="flex-1 overflow-y-auto p-8 lg:p-12 relative prose prose-invert max-w-none">
          ...
          <div className="text-zinc-300 leading-loose space-y-6 text-lg">
            ...
            <div className="my-8 bg-zinc-900 border-l-4 border-violet-500 p-6 rounded-r-xl">
        ...
        <div className="h-24 border-t border-zinc-800 bg-[#121212] px-8 flex items-center justify-between mt-auto shrink-0">
          ...
          <button 
            ...
            className="px-8 py-3 bg-gradient-to-r from-violet-600 to-purple-400 text-white font-bold uppercase tracking-wider text-sm rounded-xl hover:shadow-[0_4px_20px_rgba(124,58,237,0.4)] transition-all flex items-center gap-2"
          >
